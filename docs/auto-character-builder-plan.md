...
- 非轻型副手请求当前会被忽略, UI 还没有提示原因。
- 装备后的主手攻击会因 `refreshCharacterAutomation` 重算, 但魔法武器攻击与普通主手武器互斥关系需要更细测试。
...
状态: 进行中。阶段 4a 已完成基础装备冲突规则。

任务:

1. 已补主手, 副手, 盾牌, 双手武器冲突检查。
2. 已按规则限制副手武器为轻型, 但 UI 暂无提示。
3. 将魔法武器装备流程合并到 `equipmentRules.ts`, 避免 UI 层手写攻击计算。
