...
- 当前审计覆盖普通主手和副手武器刷新。
- 魔法武器刷新仍未纳入, 因为当前魔法武器调整绑定背包物品 ID, 不保存基础武器 ID。后续若要支持魔法武器随属性和战斗风格自动刷新, 需要先补充可恢复的基础武器绑定信息。
...
状态: 进行中。阶段 4a 已完成基础装备冲突规则, 阶段 4b 已完成魔法武器攻击生成模块化, 阶段 4c 已完成副手装备失败提示, 阶段 4d 已完成装备行为回归审计, 阶段 4e 已完成攻击数值审计增强, 阶段 4f 已完成普通武器刷新重算审计。
...
5. 已添加装备行为审计样例, 覆盖基础互斥, 攻击条目, 熟练加值, 属性加值, 箭术, 对决, 魔法武器加值, 以及普通武器在属性和战斗风格变化后的刷新重算。未完成: 魔法武器刷新重算和更完整的熟练变化重算。
