...
前后端共享自动构筑规则的完整盘点, 目标接口和迁移提交顺序见 [自动构筑共享规则核心盘点与接口设计](rules-core-extraction-plan.md).

- 当前 `packages/rules-core` 只覆盖专长候选, 先决条件和基础专长属性投影, 不能视为完整自动构筑规则包.
- 后续主线从共享规则计划 R1 开始, 先拆 catalog 和 canonical JSON model, 再迁移通用选择, 起源, 完整专长, 职业特有选择, 法术, 建卡/升级 projection 和装备派生.
- 在共享规则域完成并通过上游行为 parity 前, 不在下游服务端重复实现该规则.
...
范围: `Rune Shaper|BGG` 符文法术选择.

改动:

- `Rune Shaper|BGG` 的专长法术解析改为固定加入 `通晓语言`, 不再把全部符文法术静默加入 profile.
- `符文法术` 作为选择组暴露, 选择数量等于 `floor(熟练加值 / 2)`, 最小为 1.
- 已拥有 `Rune Shaper|BGG` 的角色在熟练加值阈值导致已知符文数量增加时, 升级流程会弹出新增符文法术选择.
- 扩展 `audit-feat-behavior`, 验证未选择时不会加入符文法术, 初始选择会加入 profile, 8 级升 9 级会要求新增 1 个符文法术并保留旧选择.
...
- XPHB `Ritual Caster|仪式施法者` 的当前等级仪式法术选择已在阶段 5e 接入, 5, 9, 13, 17 级后续新增仪式法术升级入口已在阶段 5f 接入.
...
- 本阶段只记录统一使用次数. 具体巨人先祖分支选择 UI 和分支备注已在阶段 3bt 接入.
...
- `extract:5etools` 会为专长保留固定 `darkvision`, `blindsight`, `tremorsense`, `truesight`, `resist`, `immune`, `vulnerable`, `conditionImmune` 字段.
- 专长固定感官, 抗性, 免疫, 易伤和状态免疫会通过通用 `addTextEntry` 路径写入角色卡结构化列表.
- 移除 `Boon of Truesight`, XPHB `Skulker`, `Ember of the Fire Giant`, `Fury of the Frost Giant` 的专用固定字段写入分支, 改由专长数据驱动.
- 扩展 `audit-feat-behavior`, 通过 `Infernal Constitution|XGE` 验证固定寒冷和毒素抗性会写入 `damageResistances` 和结构化说明条目.
...
- 新增 `getFeatResistanceChoiceOptions`, 从专长 `resist.choose.from` 抽取抗性候选, 并保留 `count`.
- `AutoBuilderFeatChoice` 和 ASI 专长选择新增 `featResistanceChoices`, 所选抗性通过 `addTextEntry` 写入 `damageResistances`.
- 选择型抗性会同时加入专长结构化说明条目, 并随对应 `auto-feat-...-choice-resistances` 调整撤销.
- 建卡 UI 在种族赠专长, 起源专长, 战斗风格赠专长和 ASI 专长入口显示同一抗性选择控件.
- 扩展 `audit-feat-behavior`, 验证 XPHB `Boon of Energy Resistance|能量抗性之恩惠` 暴露 2 项抗性选择, 并能写入所选强酸和雷鸣抗性.
...
- 本阶段只处理显式 `resist.choose.from` 的字符串候选. 每日长休重新选择的交互仍需要后续休息/重配接口, 临时或条件抗性仍保留在专长描述中.
...
- 扩展 `audit-feat-behavior`, 通过 `Boon of Blazing Dawn|ABH` 验证固定 `immune: 光耀` 会写入 `damageImmunities`.
- 扩展 `audit-feat-behavior`, 通过 `Boon of Poison Mastery|FRHoF` 验证固定 `immune: 毒素` 会写入 `damageImmunities`, 固定 `conditionImmune: 中毒` 会写入 `conditionImmunities`.
- 审计同时验证对应结构化说明条目存在, 防止后续只写数值字段而丢失规则来源说明.
...
- 本阶段是验证补强, 不改变运行时行为. 固定专长免疫和状态免疫的实际写入已由阶段 3bq 的通用专长结构化字段路径提供.

## 阶段 3bt 记录

状态: 已完成.

范围: XPHB 歌利亚巨人先祖具体分支选择.

改动:

- `AutoBuilderRaceChoice` 新增 `featureChoices`, 用于种族特性分支选择.
- 新增 `getRaceFeatureChoiceOptions`, 当前为 XPHB `Goliath|歌利亚` 暴露 `Giant Ancestry|巨人先祖` 的 6 个分支.
- 自动车卡 UI 在种族选择区显示巨人先祖下拉选项, 并纳入完成度校验和建卡参数.
- `Goliath|XPHB` 的巨人先祖资源备注会写入所选分支的具体效果, 如 `岚之暴鸣`.
- 后续升级刷新资源次数时会继承已有资源备注, 避免 5 级刷新后丢失具体先祖分支说明.
- 扩展 `audit-origin-structured-behavior`, 验证巨人先祖暴露 6 个选项, 选择 `storm` 后 1 级资源和 5 级刷新资源都保留 `岚之暴鸣` 备注.
...
- 本阶段只结构化建卡时的固定巨人先祖分支选择. 若后续允许重训或手动改选种族分支, 可复用 `featureChoices` 字段和资源备注生成函数.
...
范围: XPHB 仪式施法者等级门槛法术选择.

改动:

- `getFeatSpellChoiceState` 新增可选 `characterLevel` 参数, 只暴露当前等级已经开放的等级门槛专长法术选择.
- `createFeatSpellOperations` 使用同一等级过滤, 避免构建时应用未来等级才开放的专长法术选择.
- 自动车卡 UI 的种族赠专长和起源专长按 1 级过滤, 战斗风格赠专长和 ASI 专长按目标总等级过滤.
- 扩展 `audit-feat-behavior`, 验证 XPHB `Ritual Caster|仪式施法者` 4 级只暴露初始 2 个仪式法术选择, 5 级才暴露下一组选择, 且所选仪式法术进入 prepared 专长法术 profile.
...
- 本阶段只修正专长法术选择的等级门槛. 仪式施法者在 5, 9, 13, 17 级获得新增仪式法术时的升级流程入口已在阶段 5f 接入.

## 阶段 5f 记录

状态: 已完成.

范围: 已有 XPHB 仪式施法者的升级新增仪式法术选择.

改动:

- 新增 `getExistingFeatSpellLevelUpChoiceState`, 根据旧角色总等级和升级后总等级计算已有专长本级新开放的法术选择组.
- `buildLevelUpCharacter` 新增 `existingFeatChoices` 参数, 用于应用已有专长升级时新增的法术选择.
- 已有专长法术升级使用 `addSpell` 写入既有专长法术 profile, 避免覆盖已经选择的仪式法术.
- 自动车卡 UI 在升级模式下显示已有专长的新开放法术选择, 并纳入升级按钮完成度校验.
- 扩展 `audit-feat-behavior`, 验证 XPHB `Ritual Caster|仪式施法者` 从 4 级升到 5 级时只暴露 1 个新增仪式法术选择, 且升级后保留原 2 个仪式法术并加入新法术.
...
- 本阶段先覆盖当前数据中明确存在等级门槛新增选择的 XPHB `Ritual Caster|仪式施法者`. 新 helper 后续可扩展到其他已有专长的等级门槛法术选择.
...
- 专长固定感官, 抗性, 免疫, 易伤和状态免疫字段会通过通用 `addTextEntry` 路径写入结构化列表.
- 专长选择型抗性会在 UI 中选择, 并通过 `addTextEntry` 写入 `damageResistances`.
- 专长固定伤害免疫和状态免疫已有真实升级路径审计覆盖.
...
状态: 进行中. 阶段 5a 已完成分类和 prepared 标记的脚本级审计增强, 阶段 5b 已完成职业和子职额外准备法术审计, 阶段 5c 已完成专长赠法术 prepared 标记审计, 阶段 5d 已完成真实升级路径施法审计, 阶段 5e 已完成 XPHB 仪式施法者等级门槛法术选择过滤, 阶段 5f 已完成已有仪式施法者升级新增仪式法术选择.
...
- 已提取 `packages/rules-core` 作为浏览器和服务端可共同消费的纯规则边界. 第一批覆盖专长先决条件, 授权来源过滤, 来源优先级和专长属性选择; 原车卡的属性值提升专长候选已经改为调用该包. 该包不读取 catalog, 不访问 `localStorage`, 不渲染 React, 不直接修改角色, 未支持的先决条件默认拒绝.
- 后续应继续把专长的技能, 工具, 武器, 抗性, 法术等 choice state 和权威 `validateLevelUpChoice` 下沉到同一包, 避免浏览器与服务端分别实现规则.
- 共享包现已提供基础专长升级 validator: 对无需额外法术, 熟练, 抗性, 战技等子选择的专长统一复核候选资格和属性加值; 检测到未迁移的结构化字段时默认拒绝.
