...
- catalog 共 276 个专长, 覆盖 19 个来源.
- 160 个带属性规则, 205 个带先决条件, 66 个带 `additionalSpells`.
- 选择型结构包含技能 12、工具 10、武器 4、语言 7、豁免 2、专精 6、抗性 9.
- 专用选择包含 `Fighting Initiate` 1、`Eldritch Adept` 1、`Martial Adept` 1、`Metamagic Adept` 1.
- 固定结构化字段包含护甲熟练 6、盲视 3、伤害免疫 2、状态免疫 2、真实视觉 1, 以及来源特定数值、资源和等级刷新.

实施拆分:

#### R5.1 专长候选、身份和先决条件

- 扩展现有 basic feat core, 统一 276 个专长的稳定 identity、来源授权、同名优先级、重复选择和先决条件.
- 将 `getOfficialFeatOptions` 与装备侧显示名称判断迁移为共享 identity API.

R5.1 拆分为两个可独立回归的提交边界:

##### R5.1a catalog 候选授权和稳定身份

- 状态: 已完成.
- `getRuleFeatOptions` 先按调用方提供的来源或实体白名单过滤 catalog, 再执行规则版本、同名来源优先级、重复选择和已支持先决条件过滤.
- 默认策略集中维护 2014/2024 专长来源优先级; EulCauDnD 删除本地 `OFFICIAL_FEAT_SOURCES` 和 `FEAT_SOURCE_PRIORITY`.
- 已有专长从 `auto-feat-${key}-${source}` 恢复稳定 `key|source` identity, 重复判断不再依赖本地化显示名称.
- 显式授权单个实体不会放行同来源的其他实体.
- rules-core 43 项测试、Ao 全 workspace TypeScript 检查、EulCauDnD 专长行为审计和生产构建通过.

提交:

```text
refactor(rules): share feat catalog options
```
...
```text
refactor(rules): share feat eligibility
```

#### R5.2 通用专长 choice groups 和 effects
...
```text
refactor(rules): share specialized feat choices
```

#### R5.5 固定效果、资源和清理

- 状态: 已完成.
- `createRuleFeatFixedEffects` 统一 Tough、Alert、Mobile/Speedy、Squat Nimbleness、Tavern Brawler 和两项固定史诗恩惠的数值或熟练效果.
- 共享资源定义表覆盖 catalog 中 42 个专长资源实例, 保存稳定 ID、名称、上限、恢复周期和备注; 熟练加值资源通过同一 `proficiency` 上限定义计算.
- `createRuleFeatAdvancementEffects` 根据稳定专长 identity 投影 Tough 生命值增量、XPHB Alert 熟练加值阈值和所有熟练加值资源刷新, 不再在 EulCauDnD 重复枚举专长.
- canonical `combat.number.add` 和 combat modifiers 增加 `speedBonus`, 共享 apply 层保留独立速度加值, EulCauDnD adapter 继续投影为可撤销的 `speedBonus` adjustment.
- 删除本地 `makeFeatResource`、`getFeatResourceOperations`、固定数值名称特判和 20 余个已有专长升级刷新分支. `autoBuilderRules.ts` 中剩余专长匹配仅用于稳定 identity 查询; 装备和攻击行为继续使用共享 identity parser, 其规则迁移留在 R9.
- rules-core 61 项测试覆盖固定数值、来源差异、42 个资源实例、稳定 metadata、熟练加值阈值和只刷新动态资源.
- EulCauDnD 生产构建、完整专长审计、专长法术审计、装备审计和角色数据审计通过.
