...
- Controlled `AutoCharacterBuilder` embedding API with injected catalog loading, explicit creation/level-up mode, structured selections, and async host validation.
- Floating d20 roller anchored to the panel's bottom-right corner, with command results displayed from oldest to newest and the output viewport following new results.
